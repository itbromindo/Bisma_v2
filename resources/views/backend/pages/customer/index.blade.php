        ...
        function fetchSubmenus(searchQuery, page) {
            $.ajax({
                ...
                success: function (response) {
                    // Update table body
                    $('#tableBody').html('');
                    if (response.customer.data.length > 0) {
                        response.customer.data.forEach(function (customer, index) {
                            $('#tableBody').append(`
                                <tr>
                                    ...
                                    <td>${ customer.customers_full_address }</td>
                                    <td class="text-center">
                                        <div class="d-flex justify-content-center gap-2">
                                            <!-- Tombol Delete -->
                                            @if ($usr->can('customer.delete'))
                                            <button class="btn btn-light btn-sm border border-danger text-danger" title="Delete" onclick="delete_data('${customer.customer_id}')">
                                                <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                    <path d="M12.5 3.5L3.5 12.5" stroke="red" stroke-width="1.25" stroke-linecap="round" stroke-linejoin="round" />
                                                    <path d="M12.5 12.5L3.5 3.5" stroke="red" stroke-width="1.25" stroke-linecap="round" stroke-linejoin="round" />
                                                </svg>
                                            ...
                                            @endif
                                            <!-- Tombol Edit -->
                                            <button class="btn btn-light btn-sm border border-success text-success" title="Edit" onclick="showedit('${customer.customer_id}')">
                                                <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                    <path d="M12.1464 1.85355C12.3417 1.65829 12.6583 1.65829 12.8536 1.85355L14.1464 3.14645C14.3417 3.34171 14.3417 3.65829 14.1464 3.85355L5.35355 12.6464L2.5 13.5L3.35355 10.6464L12.1464 1.85355Z" stroke="green" stroke-width="1.25" stroke-linecap="round" stroke-linejoin="round" />
                                                    <path d="M11.5 2.5L13.5 4.5" stroke="green" stroke-width="1.25" stroke-linecap="round" stroke-linejoin="round" />
                                                </svg>
                        ...
                        $('#tableBody').append('<tr><td colspan="6" class="text-center">No results found</td></tr>');
                    }

                    // Update pagination
                    $('#pagination').html('');
                    response.customer.links.forEach(function (link) {
