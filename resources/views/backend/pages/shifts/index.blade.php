                                                            ...
                                                            <tr>
                                                                ...
                                                                <td class="text-center">
                                                                    <div class="d-flex justify-content-center gap-2">
                                                                        <button class="btn btn-light btn-sm border border-danger text-danger" title="Delete" onclick="delete_data('{{ $shift->shift_id }}')">
                                                                            <svg width="16" height="16" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                                                <path d="M12.5 3.5L3.5 12.5" stroke="red" stroke-width="1.25" stroke-linecap="round" stroke-linejoin="round" />
                                                                                <path d="M12.5 12.5L3.5 3.5" stroke="red" stroke-width="1.25" stroke-linecap="round" stroke-linejoin="round" />
                                                                            </svg>
                                                                        </button>
                                                                        <button class="btn btn-light btn-sm border border-success text-success" title="Edit" onclick="showedit('{{ $shift->shift_id }}')">
                                                                            <svg width="16" height="16" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        ...
                                        <nav aria-label="Page navigation">
                                            <ul class="pagination justify-content-end">
                                                <li class="page-item {{ $shifts->onFirstPage() ? 'disabled' : '' }}">
                                                    <a class="page-link" href="{{ $shifts->appends(['search' => $search ?? request('search')])->previousPageUrl() }}" aria-label="Previous">
                                                        <i class="ph-arrow-left"></i>
                                                    </a>
                                                </li>

                                                @foreach ($shifts->onEachSide(0)->appends(['search' => $search ?? request('search')])->links()->elements[0] as $page => $url)
                                                    <li class="page-item {{ $shifts->currentPage() == $page ? 'active' : '' }}">
                                                        <a class="page-link" href="{{ $url }}">{{ sprintf('%02d', $page) }}</a>
                                                    </li>
                                                @endforeach

                                                <li class="page-item {{ $shifts->hasMorePages() ? '' : 'disabled' }}">
                                                    <a class="page-link" href="{{ $shifts->appends(['search' => $search ?? request('search')])->nextPageUrl() }}" aria-label="Next">
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form>
                    ...
                    <div class="formGroup mb-3">
                        <label>Notes</label>
                        <textarea class="form-control" name="shift_notes" id="shift_notes" placeholder="Notes"></textarea>
                    </div>
                    <div class="formGroup mb-3">
                        <label>Start Time Before Break</label>
                        <input class="form-control" type="time" id="shift_start_time_before_break" placeholder="Start Time Before Break" />
                    </div>
                    <div class="formGroup mb-3">
                        <label>End Time Before Break</label>
                        <input class="form-control" type="time" id="shift_end_time_before_break" placeholder="End Time Before Break" />
                    </div>
                    <div class="formGroup mb-3">
                        <label>Start Time After Break</label>
                        <input class="form-control" type="time" id="shift_start_time_after_break" placeholder="Start Time After Break" />
                    </div>
                    <div class="formGroup mb-3">
                        <label>End Time After Break</label>
                        <input class="form-control" type="time" id="shift_end_time_after_break" placeholder="End Time After Break" />
                    </div>
