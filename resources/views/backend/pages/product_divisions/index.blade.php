                ...
                <div class="col-xxl-12 col-xl-12 col-md-12">
                    <div class="card">
                        <div class="card-header d-flex justify-content-between align-items-center">
                            <h5>Master Divisi Produk</h5>
                            <div class="d-flex align-items-center">
                                <div class="app-main-search me-2">
                                    <form action="/admin/product_divisions" method="GET" class="d-flex">
                                ...
                                @if ($usr->can('product_divisions.create'))
                                                            ...
                                                            <tr>
                                                                ...
                                                                <td class="text-center">
                                                                    <div class="d-flex justify-content-center gap-2">
                                                                        <!-- Tombol Delete -->
                                                                        @if ($usr->can('product_divisions.delete'))
                                                                        <button class="btn btn-light btn-sm border border-danger text-danger" title="Delete" onclick="delete_data('{{ $pds->product_divisions_id }}')">
                                                                        ...
                                                                        @endif
                                                                        <!-- Tombol Edit -->
                                                                        <button class="btn btn-light btn-sm border border-success text-success" title="Edit" onclick="showedit('{{ $pds->product_divisions_id }}')">
                                                                            <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    ...
                                    <div class="col-lg-12">
                                        <nav aria-label="Page navigation">
                                            <ul class="pagination justify-content-end" id="pagination">
                                                <!-- Previous Page Link -->
                                                <li class="page-item {{ $product_divisions->onFirstPage() ? 'disabled' : '' }}">
                                                    <a class="page-link" href="{{ $product_divisions->appends(['search' => $search ?? request('search')])->previousPageUrl() }}" aria-label="Previous">
                                                        <i class="ph-arrow-left"></i>
                                                    </a>
                                                </li>

                                                <!-- Page Number Links -->
                                                @foreach ($product_divisions->onEachSide(0)->appends(['search' => $search ?? request('search')])->links()->elements[0] as $page => $url)
                                                    <li class="page-item {{ $product_divisions->currentPage() == $page ? 'active' : '' }}">
                                                        <a class="page-link" href="{{ $url }}">{{ sprintf('%02d', $page) }}</a>
                                                    </li>
                                                @endforeach

                                                <!-- Next Page Link -->
                                                <li class="page-item {{ $product_divisions->hasMorePages() ? '' : 'disabled' }}">
                                                    <a class="page-link" href="{{ $product_divisions->appends(['search' => $search ?? request('search')])->nextPageUrl() }}" aria-label="Next">
    ...
    <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                ...
                @endif
                @if ($usr->can('product_divisions.update') || $usr->can('product_divisions.create'))
...
<script>
        ...
        function fetchSubmenus(searchQuery, page) {
            $.ajax({
                ...
                success: function (response) {
                    // Update table body
                    $('#tableBody').html('');
                    if (response.product_divisions.data.length > 0) {
                        response.product_divisions.data.forEach(function (product_divisions, index) {
                            $('#tableBody').append(`
                                <tr>
                                    ...
                                    <td>${ product_divisions.product_divisions_notes }</td>
                                    <td class="text-center">
                                        <div class="d-flex justify-content-center gap-2">
                                            <!-- Tombol Delete -->
                                            @if ($usr->can('product_divisions.delete'))
                                            <button class="btn btn-light btn-sm border border-danger text-danger" title="Delete" onclick="delete_data('${product_divisions.product_divisions_id}')">
                                                <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                    <path d="M12.5 3.5L3.5 12.5" stroke="red" stroke-width="1.25" stroke-linecap="round" stroke-linejoin="round" />
                                                    <path d="M12.5 12.5L3.5 3.5" stroke="red" stroke-width="1.25" stroke-linecap="round" stroke-linejoin="round" />
                                                </svg>
                                            ...
                                            @endif
                                            <!-- Tombol Edit -->
                                            <button class="btn btn-light btn-sm border border-success text-success" title="Edit" onclick="showedit('${product_divisions.product_divisions_id}')">
                                                <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                    <path d="M12.1464 1.85355C12.3417 1.65829 12.6583 1.65829 12.8536 1.85355L14.1464 3.14645C14.3417 3.34171 14.3417 3.65829 14.1464 3.85355L5.35355 12.6464L2.5 13.5L3.35355 10.6464L12.1464 1.85355Z" stroke="green" stroke-width="1.25" stroke-linecap="round" stroke-linejoin="round" />
                                                    <path d="M11.5 2.5L13.5 4.5" stroke="green" stroke-width="1.25" stroke-linecap="round" stroke-linejoin="round" />
                                                </svg>
                    ...
                    // Update pagination
                    $('#pagination').html('');
                    response.product_divisions.links.forEach(function (link) {
    ...
    function clearform() {
        document.getElementById('product_divisions_id').value = '';
        document.getElementById('product_divisions_name').value = '';
        document.getElementById('product_divisions_notes').value = '';
