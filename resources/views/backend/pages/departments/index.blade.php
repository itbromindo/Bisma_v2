                                                    ...
                                                    <tbody id="tableBody">
                                                        @foreach ($departments as $department)
                                                            ...
                                                            <td scope="row" class="text-center">
                                                                ...
                                                                <td class="text-center">{{ Str::words($department->department_code, 10, '...') }}</td>
                                                                <td class="text-left">{{ Str::words($department->department_name, 10, '...') }}</td>
                                                                <td class="text-left">{{ Str::words($department->department_notes, 10, '...') }}</td>
                                                                <td class="text-center">{{ $department->division->division_name ?? '-' }}</td>
                                                                <td class="text-center">
                                                                    <div class="d-flex justify-content-center gap-2">
                                                                        @if ($usr->can('department.delete'))
            ...
            <div class="modal-body">
                <form>
                    ...
                    <div class="modal-footer">
                        ...
                        @endif
                        @if ($usr->can('department.update') || $usr->can('department.create'))
        ...
        $('#search').on('keyup', function () {
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    $('#tableBody').html('');
                    if (response.departments.data.length > 0) {
                        response.departments.data.forEach(function (department, index) {
                            $('#tableBody').append(`
                                <tr>
                                    <td class="text-center">${(response.departments.current_page - 1) * response.departments.per_page + index + 1}</td>
                                    <td class="text-center">${ truncateText(department.department_code, 10, '...')}</td>
                                    <td class="text-left">${ truncateText(department.department_name, 10, '...')}</td>
                                    <td class="text-left">${ truncateText(department.department_notes, 10, '...') ?? ''}</td>
